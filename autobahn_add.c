...
/**
 * @brief Perform binary addition (single_add) on words with carry.
 *
...
static void single_add(bi_uword* z, bi_uword* c, const bi_uword* x, const bi_uword* y)
{
...
static void bi_uadd(bi** z, const bi* x, bi* y)
{
    ...
    // Perform binary addition (single_add) word by word
    for (int j = 0; j < x->dmax; j++)
        single_add(&(sum->a[j]), &c, &(x->a[j]), &(y->a[j]));
